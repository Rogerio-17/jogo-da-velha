let x = document.querySelector(".x");
let o = document.querySelector(".o");
let boxes = document.querySelectorAll(".box");
let bottons = document.querySelectorAll("#buttons-container button");
let message = document.querySelector("#message");
let messageText = document.querySelector("#message p");
let secondPlayer;

// contador de jogadas
let player1 = 0;
let player2 = 0;

// adicionando evento de click
for (let i = 0; i < boxes.length; i++) {
  boxes[i].addEventListener("click", function () {
    //Verifica se o campo já possue elementos
    if (this.childNodes.length == 0) {
      let el;
      //Verifica quem vai jogar
      if (player1 == player2) {
        //o
        el = o;
        // count player 1
        player1++;

        if (secondPlayer == "ai-player") {
          //função execulta a jogada
          computerPlayer();
          player2++;
        }
      } else {
        //x
        el = x;
        // count player 2
        player2++;
      }
      let cloneEl = el.cloneNode(true);
      this.appendChild(cloneEl);
      checkWinCondition();
      // Mensagem de erro caso possua elementos
    } else {
      alert("Não é possivel jogar nesse campo");
    }
  });
}

//Evento para saber se é dois players ou jogar contra a AI
for (let i = 0; i < bottons.length; i++) {
  bottons[i].addEventListener("click", function () {
    for (let x = 0; x < bottons.length; x++) {
      secondPlayer = this.getAttribute("id");
      bottons[x].style.display = "none";
    }

    setTimeout(function () {
      let container = document.querySelector("#container");
      container.classList.remove("hide");
    }, 500);
  });
}

//Checa quem venceu
function checkWinCondition() {
  let b1 = document.getElementById("block-1");
  let b2 = document.getElementById("block-2");
  let b3 = document.getElementById("block-3");
  let b4 = document.getElementById("block-4");
  let b5 = document.getElementById("block-5");
  let b6 = document.getElementById("block-6");
  let b7 = document.getElementById("block-7");
  let b8 = document.getElementById("block-8");
  let b9 = document.getElementById("block-9");

  //Verifica que venceu na primeira horizontal
  if (
    b1.childNodes.length > 0 &&
    b2.childNodes.length > 0 &&
    b3.childNodes.length > 0
  ) {
    let b1child = b1.childNodes[0].className;
    let b2child = b2.childNodes[0].className;
    let b3child = b3.childNodes[0].className;

    if (b1child == "o" && b2child == "o" && b3child == "o") {
      // o venceu
      declareWinner("o");
    } else if (b1child == "x" && b2child == "x" && b3child == "x") {
      declareWinner("x");
    }
  }

  // Verifica quem venceu na segunda horizontal
  if (
    b4.childNodes.length > 0 &&
    b5.childNodes.length > 0 &&
    b6.childNodes.length > 0
  ) {
    let b4child = b4.childNodes[0].className;
    let b5child = b5.childNodes[0].className;
    let b6child = b6.childNodes[0].className;

    if (b4child == "o" && b5child == "o" && b6child == "o") {
      // o venceu
      declareWinner("o");
    } else if (b4child == "x" && b5child == "x" && b6child == "x") {
      // x venceu
      declareWinner("x");
    }
  }

  // Verifica quem venceu na Terceria horizontal
  if (
    b7.childNodes.length > 0 &&
    b8.childNodes.length > 0 &&
    b9.childNodes.length > 0
  ) {
    let b7child = b7.childNodes[0].className;
    let b8child = b8.childNodes[0].className;
    let b9child = b9.childNodes[0].className;

    if (b7child == "o" && b8child == "o" && b9child == "o") {
      // o venceu
      declareWinner("o");
    } else if (b7child == "x" && b8child == "x" && b9child == "x") {
      // x venceu
      declareWinner("x");
    }
  }
  // Verifica quem venceu na primeira vertical
  if (
    b1.childNodes.length > 0 &&
    b4.childNodes.length > 0 &&
    b7.childNodes.length > 0
  ) {
    let b1child = b1.childNodes[0].className;
    let b4child = b4.childNodes[0].className;
    let b7child = b7.childNodes[0].className;

    if (b1child == "o" && b4child == "o" && b7child == "o") {
      // o venceu
      declareWinner("o");
    } else if (b1child == "x" && b4child == "x" && b7child == "x") {
      // x venceu
      declareWinner("x");
    }
  }

  // Verifica quem venceu na seguda vertical
  if (
    b2.childNodes.length > 0 &&
    b5.childNodes.length > 0 &&
    b8.childNodes.length > 0
  ) {
    let b2child = b2.childNodes[0].className;
    let b5child = b5.childNodes[0].className;
    let b8child = b8.childNodes[0].className;

    if (b2child == "o" && b5child == "o" && b8child == "o") {
      // o venceu
      declareWinner("o");
    } else if (b2child == "x" && b5child == "x" && b8child == "x") {
      // x venceu
      declareWinner("x");
    }
  }
  // Verifica quem venceu na terceira vertical
  if (
    b3.childNodes.length > 0 &&
    b6.childNodes.length > 0 &&
    b9.childNodes.length > 0
  ) {
    let b3child = b3.childNodes[0].className;
    let b6child = b6.childNodes[0].className;
    let b9child = b9.childNodes[0].className;

    if (b3child == "o" && b6child == "o" && b9child == "o") {
      // o venceu
      declareWinner("o");
    } else if (b3child == "x" && b6child == "x" && b9child == "x") {
      // x venceu
      declareWinner("x");
    }
  }

  // Verifica quem venceu na primera paralelo
  if (
    b1.childNodes.length > 0 &&
    b5.childNodes.length > 0 &&
    b9.childNodes.length > 0
  ) {
    let b1child = b1.childNodes[0].className;
    let b5child = b5.childNodes[0].className;
    let b9child = b9.childNodes[0].className;

    if (b1child == "o" && b5child == "o" && b9child == "o") {
      // o venceu
      declareWinner("o");
    } else if (b1child == "x" && b5child == "x" && b9child == "x") {
      // x venceu
      declareWinner("x");
    }
  }

  // Verifica quem venceu na segundo paralelo
  if (
    b3.childNodes.length > 0 &&
    b5.childNodes.length > 0 &&
    b7.childNodes.length > 0
  ) {
    let b3child = b3.childNodes[0].className;
    let b5child = b5.childNodes[0].className;
    let b7child = b7.childNodes[0].className;

    if (b3child == "o" && b5child == "o" && b7child == "o") {
      // o venceu
      declareWinner("o");
    } else if (b3child == "x" && b5child == "x" && b7child == "x") {
      // x venceu
      declareWinner("x");
    }
  }

  // verifica se deu velha
  let count = 0;

  for (let i = 0; i < boxes.length; i++) {
    if (boxes[i].childNodes[0] != undefined) {
      count++;
    }
  }

  if (count == 9) {
    declareWinner("velha");
  }
}

// Limpa o jogo, informa o vencedor e atualiza o placar
function declareWinner(winner) {
  let scoreX = document.querySelector("#scoreboard-1");
  let scoreO = document.querySelector("#scoreboard-2");
  let msg = "";

  if (winner == "o") {
    scoreO.textContent = parseInt(scoreO.textContent) + 1;
    msg = "Jogador 1 venceu!";
  } else if (winner == "x") {
    scoreX.textContent = parseInt(scoreX.textContent) + 1;
    msg = "Jogador 2 venceu!";
  } else {
    msg = "Deu velha";
  }

  //exibir msg
  messageText.innerHTML = msg;
  message.classList.remove("hide");

  //esconde a mensagem
  setTimeout(function () {
    message.classList.add("hide");
  }, 2300);

  // Zera jogadas
  player1 = 0;
  player2 = 0;

  // remove x e o
  let boxsRemove = document.querySelectorAll(".box div");
  for (let i = 0; i < boxsRemove.length; i++) {
    boxsRemove[i].parentNode.removeChild(boxsRemove[i]);
  }
}

// Boot
function computerPlayer() {
  let clonex = x.cloneNode(true);
  counter = 0;
  filled = 0;

  for (let i = 0; i < boxes.length; i++) {
    let randomNumber = Math.floor(Math.random() * 5);

    // Preenche o campo quando o filho esta vazio
    if (boxes[i].childNodes[0] == undefined) {
      console.log(boxes[i].childNodes[0]);
      if (randomNumber <= 1) {
        boxes[i].appendChild(clonex);
        counter++;
      }
      // Checagem de quantas estão preenchidas
    } else {
      filled++;
    }
  }

  if (counter == 0 && filled < 9) {
    computerPlayer();
    console.log(counter);
  }
}
